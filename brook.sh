...
Service_brook() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate "https://raw.githubusercontent.com/Kurisu-public/brook-server-script/master/service/brook_centos" -O /etc/init.d/brook; then
    ...
    if ! wget --no-check-certificate "https://raw.githubusercontent.com/Kurisu-public/brook-server-script/master/service/brook_debian" -O /etc/init.d/brook; then
      echo -e "${Error} Brook服务 管理脚本下载失败 !" && rm -rf "${file}" && exit 1
    fi
    chmod +x "/etc/init.d/brook"
...
Set_port() {
  while true; do
    ...
    read -e -p "(默认: 51324):" bk_port
    [[ -z "${bk_port}" ]] && bk_port="51324"
...
Set_passwd() {
  ...
  read -e -p "(默认: iCpoEcgW*_dfXHkF*d9tcKsV):" bk_passwd
  [[ -z "${bk_passwd}" ]] && bk_passwd="iCpoEcgW*_dfXHkF*d9tcKsV"
  echo && echo "========================"
...
Update_Shell() {
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Kurisu-public/brook-server-script/master/brook.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1) && sh_new_type="github"
  [[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
  if [[ -e "/etc/init.d/brook" ]]; then
    rm -rf /etc/init.d/brook
    Service_brook
  fi
  wget -N --no-check-certificate "https://raw.githubusercontent.com/Kurisu-public/brook-server-script/master/brook.sh" && chmod +x brook.sh
